         ...
         columnDefs: [
            
            {
                ...
                render: function(data, type, row) {
                    ...
                    if(status==0){
                          return `<button type="button" class="btn btn-danger decenviada" data-id="${row.id}"> Enviar</button>`;
                    }else if(status==1){
...
var mes = meses[fechaParseada.getMonth()];
                        ...
                        return `<button type="button" class="btn btn-success decentregada" data-id="${row.id}">
...
$('#usuariosTable').on('click', '.decenviada', function() {
    ...
    Swal.fire({
        title: '¿Está seguro?',
        text: '¿Desea marcar esta declaración como enviada para su entrega?',
        icon: 'question',
        ...
        if (result.value)  if (result.value) {
               $.ajax({
                url: 'php/declaracion.php', // Reemplaza con la URL de tu archivo PHP
                method: 'POST',
                data: {
                    function: 'actualizaStatus',
                    id_update: id_declaracion,
                    status: '1', // Suponiendo que este es el nuevo estado
                    comentario: result.value // Comentario opcional
                },
                dataType: 'json',
...
$('#usuariosTable').on('click', '.decentregada', function() {
    ...
    Swal.fire({
        title: '¿Está seguro?',
        text: '¿Desea marcar esta declaración como entregada y finalizada?',
        icon: 'question',
        showCancelButton: true,
        confirmButtonText: 'Sí',
        cancelButtonText: 'No',
        ...
        if (result.value) {
               $.ajax({
                ...
                data: {
                    function: 'actualizaStatus',
                    id_update: id_declaracion,
                    status: '2', // Suponiendo que este es el nuevo estado
                   comentario: result.value // Comentario opcional
                },
                dataType: 'json',
                success: function(response) {
